                            ...
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach ($cuti as $key => $k)
                                        <tr
                                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            <th scope="row"
                                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                {{ $cuti->perPage() * ($cuti->currentPage() - 1) + $key + 1 }}
                                            </th>
                                            <td class="px-6 py-4">
                                                {{ $k->nama }}
                                            </td>
                                            ...
                                            <td class="px-6 py-4">
                                                ...
                                                <button
                                                    onclick="return cutiDelete('{{ $k->id }}','{{ $k->nama }}')"
...
<script>
    const editSourceModal = (button) => {
        ...
        document.getElementById('title_source').innerText = `UPDATE CUTI ${nama}`;
    ...
    const cutiDelete = async (id, nama) => {
        let tanya = confirm(`Apakah anda yakin untuk menghapus Cuti ${nama} ?`);
        if (tanya) {
            try {
                const response = await axios.post(`/cuti/${id}`, {
                    '_method': 'DELETE',
                ...
                if (response.status === 200) {
                    alert('Cuti berhasil dihapus');
                    location.reload();
                } else {
                    alert('Gagal menghapus cuti. Silakan coba lagi.');
                }
            } catch (error) {
                console.error(error);
                alert('Terjadi kesalahan saat menghapus cuti. Silakan cek konsol untuk detail.');
            }
        }
    };
</script>
