                            ...
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach ($jabatan as $key => $k)
                                        <tr
                                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            <th scope="row"
                                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                {{ $jabatan->perPage() * ($jabatan->currentPage() - 1) + $key + 1 }}
                                            </th>
                                            <td class="px-6 py-4">
                                                {{ $k->departemen->nama }}
                                            </td>
                                            ...
                                            <td class="px-6 py-4">
                                                <button type="button" data-id="{{ $k->id }}"
                                                    ...
                                                    data-id_departemen="{{ $k->id_departemen }}"
                                                    data-level="{{ $k->level }}" onclick="editSourceModal(this)"
                                                ...
                                                <button
                                                    onclick="return jabatanDelete('{{ $k->id }}','{{ $k->departemen->nama }}')"
    ...
    const jabatanDelete = async (id, jabatan) => {
        ...
        if (tanya) {
            try {
                ...
                console.error(error);
                alert('Terjadi kesalahan saat menghapus jabatan. Silakan cek konsol untuk detail.');
            }
        }
    };
</script>
